...
public class FireTimerManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (!isRunning || theGameStateManager == null || theGameStateManager.currentState != FireGameState.Play) return;
    ...
    private IEnumerator StartTimerCoroutine(float time)
    {
        ...
        theGameStateManager.SetState(FireGameState.Play); //게임 진행 중으로 상태 업데이트
