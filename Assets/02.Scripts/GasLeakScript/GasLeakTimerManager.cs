...
public class GasLeakTimerManager : MonoBehaviour
{
   public float timeLimit = 10f;   //시간 제한(10초로 임시 설정)
    ...
    void Update()
    {
        ...
        if (!isRunning || theGameStateManager == null || theGameStateManager.currentState != GasLeakGameState.Play) return;
    ...
    private IEnumerator StartTimerCoroutine(float time)
    {
        ...
        theGameStateManager.SetState(GasLeakGameState.Play); //게임 진행 중으로 상태 업데이트
